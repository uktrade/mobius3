...
def Syncer(
    ...
    async def start():
        ...
        download_directory = PurePosixPath(tempfile.mkdtemp())
        upload_tasks = [
    ...
    def watch_and_upload_directory(logger, path, mask):
        ...
        if PurePosixPath(path) != directory:
    ...
    def read_events(parent_logger):
        ...
        while offset < len(raw_bytes):
            ...
            item_type = \
                ...
                'download' if full_path.parent == download_directory else \
    ...
    def schedule_download(logger, path):
        async def download():
            ...
            temporary_path = download_directory / uuid.uuid4().hex
            is_directory = path[-1] == '/'
            try:
                if is_directory:
